...
async function main() {
  ...
  const storeInstance = await TokenStore.deploy();

  await storeInstance.deployed();

  console.log("TokenStore deployed to: ", storeInstance.address);

  const Reentrancy = await ethers.getContractFactory("Reentrancy");
  const reentrancy = await Reentrancy.deploy(storeInstance.address);
